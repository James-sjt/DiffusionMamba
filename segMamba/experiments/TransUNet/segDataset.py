...
if __name__ == '__main__':
    ...
    for idx, batch in enumerate(loader):
        ...
        print(img.shape)
        axis[0].imshow(img[0].detach().permute(1, 2, 0).cpu().numpy(), cmap='gray')
        axis[1].imshow(mask[0].detach().permute(1, 2, 0).cpu().numpy(), cmap='gray')
        # axis[2].imshow(enhanced[0].detach().permute(1, 2, 0).cpu().numpy(), cmap='gray')
        plt.show()
        plt.close(fig)
