...
class VSSM(nn.Module):
    def __init__(
        ...
        self.classifier = nn.Sequential(OrderedDict(
            ...
            head=nn.Conv2d(self.num_features, num_classes, kernel_size=3, padding=1, stride=1),
        ))
...
def vanilla_vmamba_tiny(pretrained=False, **kwargs):
    model = VSSM(
        ...
        patch_size=4, in_chans=1, num_classes=1,
...
if __name__ == "__main__":
    ...
    dataset = ImageDataset('valid', device, imgType="original")
    loader = DataLoader(dataset, batch_size=1, shuffle=True)
    model = vanilla_vmamba_tiny().to(device)
